...
Chart `1.0.0` and later target upstream `v1.0.0`, which rewrote the application. It shares no data model with what chart `0.x` deployed, so there is no upgrade path from `0.x`: install it fresh. Chart `2.0.0` replaced the Ingress with a Gateway API HTTPRoute; see the route notes below.
