...
releases. It polls that repo's releases every 6 hours (and on manual `workflow_dispatch`), and
when the chart's `appVersion` is behind it opens a PR that bumps `Chart.yaml`. When the app's
